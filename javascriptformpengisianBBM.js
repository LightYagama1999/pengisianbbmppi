function doPost(e) {
  try {
    ...
    return ContentService.createTextOutput( // kembali ke hasil JSON behasil
      JSON.stringify({
        "Terima Kasih": "Data telah terkirim",
...
function setup() {
  ...
  SCRIPT_PROP.setProperty(
    "1keoJOnsWqa2DJJye05lCrKHWEU8eRA2QxzSgog5znvc",
    doc.getId()
  );
}
/**
...
function uploadFileToGoogleDrive(data, file, nama, email, e) {
  try {
    var dropbox = "Hasil Foto Pengisian BBM";
    var folder,
    ...
    return ContentService.createTextOutput( // kembali ke hasil JSON behasil.
      JSON.stringify({ "Maaf!": "Upload data gagal!", data: JSON.stringify(f) })
    )
      .setMimeType(ContentService.MimeType.JSON);
  }
}
